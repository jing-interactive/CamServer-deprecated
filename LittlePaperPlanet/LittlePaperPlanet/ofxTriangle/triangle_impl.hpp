...
void transfernodes(struct mesh *m, struct behavior *b, REAL *pointlist,
                   ...
                   int numberPoints, int numberPointattribs)
#else /* not ANSI_DECLARATORS */
void transfernodes(m, b, pointlist, pointattriblist, pointmarkerlist,
                   numberPoints, numberPointattribs)
struct mesh *m;
struct behavior *b;
REAL *pointlist;
...

{
  ...
  m->invertices = numberPoints;
  m->mesh_dim = 2;
  m->nextras = numberPointattribs;
  m->readnodefile = 0;
  if (m->invertices < 3) {
    printf("Error:  Input must have at least three input vertices.\n");
    triexit(1);
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    /* Read the vertex attributes. */
    for (j = 0; j < numberPointattribs; j++) {
...

{
  ...
  transfernodes(&m, &b, in->pointlist, in->pointattributelist,
                in->pointmarkerlist, in->numberPoints,
                in->numberPointattributes);
#else /* not TRILIBRARY */
...
#ifdef TRILIBRARY
  if (b.jettison) {
    out->numberPoints = m.vertices.items - m.undeads;
  } else {
    out->numberPoints = m.vertices.items;
  }
  out->numberPointattributes = m.nextras;
  ...
  if (vorout != (struct triangulateio *) NULL) {
    vorout->numberPoints = m.triangles.items;
    vorout->numberPointattributes = m.nextras;
