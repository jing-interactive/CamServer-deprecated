...
public:
	...
	void addVertex(b2Vec2 pt) {
		vertices.push_back(pt);
	}
	
	//------------------------------------------------
	void setup(b2World * b2dworld, float x, float y, bool isFixed = false) {
	...
	bool validateShape() {
		...
		std::vector<b2Vec2> vertexArray(count);
		
		for(int i=0; i<count; i++) {
			vertexArray[i].Set(vertices[i].x, vertices[i].y);
		}
		b2Vec2 center = computeCentroid(&vertexArray[0], count);
		...
		b2Vec2 m_centroid = computeCentroid(&vertexArray[0], count);//ComputeCentroid(vertexArray, vertexCount);
